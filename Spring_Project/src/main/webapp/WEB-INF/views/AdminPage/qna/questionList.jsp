							...
							<tbody>
								<c:if test="${not empty qnaList and pageInfo.getListCount() > 0}">
									<c:forEach var="qna" items="${qnaList }" varStatus="status">
										<tr onclick="location.href='CommunityDetail?qna_num=${qna.getQna_num() }&page=${pageNum}'" style="cursor:pointer;">
											<td>${listCount -(listCount -((pageNum-1)* listLimit + status.index)-1)}
											</td>
											<td>
											<!-- 답글에 대한 들여쓰기(공백 추가) 작업 처리 -->
<%-- 											<c:if test="${msg eq 'QnA'}"><c:forEach var="i"begin="1" end="${qna.getQna_re_lev() }">&nbsp;</c:forEach></c:if> --%>
											${qna.getQna_title() }</td>
